...
const { authenticateToken } = require("../middleware/AuthMiddleware"); // doit définir req.user
...
router.post("/", authenticateToken, async (req, res) => {
  console.log("[ORDERS] POST /api/orders - user:", req.user?.userId || req.user?.id, "body:", req.body);
  try {
...
router.get("/", authenticateToken, async (req, res) => {
  ...
  restaurantName: plain.menu?.restaurant?.name || plain.restaurantName || null,
...
router.get("/client/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (meId !== targetId && me.role !== "admin" && me.role !== "owner") {
